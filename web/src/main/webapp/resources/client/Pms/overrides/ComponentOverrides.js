    ...
    listeners: {
        afterrender: function (panel) {
            if (panel.placeholder) {
                panel.placeholder.el.on('click', function () {
...
Ext.define('MyApp.view.override.Grid', {
    ...
    downloadExcelXml: function (includeHidden, title) {
        if (!title) title = this.up('tabpanel') ? this.up('tabpanel').activeTab.title : this.up('window').title;
...
//        title = Ext.Date.format(new Date(), 'Y-m-d H:i:s');
//
...
//            });
//
////            window.open(location, 'filename.txt');
//
//            el.click();
//
//            Ext.fly(el).destroy();
...
//        }
    ...
    createWorksheet: function (includeHidden, theTitle) {
        ...
        for (var i = 0; i < colCount; i++) {
            if (cm[i].xtype != 'actioncolumn' && cm[i].xtype != 'rownumberer' && cm[i].xtype != 'booleancolumn' && (cm[i].dataIndex != '') && (includeHidden || !cm[i].hidden)) {
                ...
                if (cm[i].text === "") {
                    ...
                    var fld = me.getModelField(cm[i].dataIndex);
                    if (fld) {
        ...
        for (var i = 0, it = this.store.data.items, l = it.length; i < l; i++) {
            ...
            var k = 0;
            for (var j = 0; j < colCount; j++) {
                if (cm[j].xtype != 'actioncolumn' && cm[j].xtype != 'rownumberer' && cm[j].xtype != 'booleancolumn' && (cm[j].dataIndex != '') && (includeHidden || !cm[j].hidden)) {
                    ...
                    if (cellType[k] !== "None") {
                        ...
                        if (cellType[k] == 'DateTime') {
                            ...
                            if (cm[j].renderer) {
                                var rendV = cm[j].renderer(v, null, it[i]);
                                t += (!rendV || rendV == '&mdash;') ? '_' : rendV;
                            }
                            else {
                                t += v ? v : '_';
                            }
