...
Ext.define('Sch.util.ScrollManager', {
    ...
    doScroll: function () {
        ...
        scrollProcessEl.scroll(scrollProcess.dir[0], this.increment, {
    ...
    onMouseMove: function (e) {
        ...
        if (region && region.contains(pt) && el.isScrollable()) {
            if (region.bottom - y <= me.vthresh && (elDom.scrollHeight - elDom.scrollTop - elDom.clientHeight > 0)) {

                if (scrollProcess.el != el) {
                    this.startScrollInterval(el, "down");
                }
                return;
            } else if (region.right - x <= me.hthresh && (elDom.scrollWidth - elDom.scrollLeft - elDom.clientWidth > 0)) {
                if (scrollProcess.el != el) {
...
Ext.define("Sch.feature.DragCreator", {
    ...
    onDragStart: function () {
        ...
        Sch.util.ScrollManager.register(me.schedulerView.el);
    },
    ...
    onDragEnd: function (tracker, e) {
        ...
        Ext.fly(t).on('click', this.eventSwallower);

        setTimeout(function () {
            Ext.fly(t).un('click', this.eventSwallower);
        }, 100);
    ...
    setupTooltips: function () {
        var me = this,
            sv = me.schedulerView,
            secondaryCanvas = sv.getSecondaryCanvasEl();
...
Ext.define("Sch.feature.SchedulerDragZone", {
    ...
    onDragOver: function (e, id) {
        ...
        if (this.showExactDropPosition) {
            var timeDiff = view.getDateFromCoordinate(e.getXY()[ 0 ]) - dd.sourceDate;
            var realStart = new Date(dd.origStart - 0 + timeDiff);
            var offsetX = view.timeAxisViewModel.getDistanceBetweenDates(realStart, dd.startDate);
    ...
    updateDragContext: function (e) {
        ...
        if (s.timeAxis.isContinuous()) {
            if (
                ...
                dd.startDate = s.timeAxis.roundDate(new Date(dd.origStart - 0 + dd.timeDiff), s.snapRelativeToEventStartDate ? dd.origStart : false);
    ...
    onDragDrop: function (e, id) {
        ...
        s.el.un('scroll', me.onViewElScroll, me);
    },
...
Ext.define("Sch.feature.ResizeZone", {
        ...
        if (this.showTooltip) {
            if (!this.tip) {
                this.tip = Ext.create("Sch.tooltip.Tooltip", {
                    schedulerView: s,
                    renderTo: s.getSecondaryCanvasEl(),
                    cls: 'sch-resize-tip'
                });
            }
    ...
    createResizer: function (el, eventRecord, handlePos) {
        ...
        Sch.util.ScrollManager.register(s.el);
    ...
    getStartEndDates: function (xy) {
        ...
        if (isStart) {
            ...
            if (s.snapRelativeToEventStartDate) {
                start = s.getDateFromXY([s.rtl ? rEl.getRight() : rEl.getLeft() + 1, rEl.getTop()]);
            ...
            if (s.snapRelativeToEventStartDate) {
                end = s.getDateFromXY([s.rtl ? rEl.getLeft() : rEl.getRight(), rEl.getBottom()]);
    ...
    partialResize: function (r, width, height, e) {
        ...
        if (s.isHorizontal()) {
            r.target.el.setY(this.resizer.constrainTo.top - s.getScroll().top + this.startScroll.top);
        }
        ...
        if (this.showExactResizePosition) {
            ...
            if (r.isStart) {
                ...
                var cursorDate = s.getDateFromCoordinate(r.otherEdgeX - Math.min(width, r.maxWidth)) || start;
                var offset = s.timeAxisViewModel.getDistanceBetweenDates(cursorDate, start);
    ...
    afterResize: function (r, w, h, e) {
        ...
        Sch.util.ScrollManager.unregister(sv.el);
    ...
    finalize: function (updateRecord) {
        ...
        context.eventRecord.store.on('update', function () {
            wasChanged = true;
        }, null, { single: true });
...
Ext.define("Sch.view.HorizontalTimeAxis", {
    ...
    headerHtmlRowTpl: '<table border="0" cellspacing="0" cellpadding="0" style="width: {totalWidth}px; {tstyle}" class="{{tableCls}} sch-header-row-{position} {cls}">' +
        '<thead>' +
        '<tr>' +
        '<tpl for="cells">' +
...
//                    cls     : this.store.buffered ? 'sch-locked-buffered-spacer' : '',
    ...
    patchSubGrid: function (grid, isLocked) {
        ...
        grid.on({
            collapse: function () {
                bufferedRenderer.disabled = true;
            },
            expand: function () {
                bufferedRenderer.disabled = false;
            }
        });
...
Ext.define("Sch.data.FilterableNodeStore", {
    ...
    onNodeCollapse: function (parent, records, suppressEvent, callback, scope) {
        ...
        data.contains = function () {
            ...
            for (var i = 0; i < records.length; i++)
                if (!records[ i ].hidden && prevContains.call(this, records[ i ])) {
...
/**
 ...
 Filtering of a tree store is different from filtering flat stores. In a flat store, all nodes (items) are of the same type and on the same hierarchical level.
 Filtering can hide any nodes that not matching some criteria.
 ...
 And usually a "leaf" node can't be sufficiently identified w/o its parents - i.e. it is important to know all the parents that a particular leaf node belongs to. So when filtering
    ...
    reconfigure: function (config, suppressRefresh) {
        ...
        if (this.fireEvent('beforereconfigure', this, start, end) !== false) {
            ...
            if (this.isContinuous()) {
                ...
                this.adjustedEnd = DATE.getNext(count > 1 ? ticks[ count - 1 ].start : adjusted.start, unit, increment);
            } else {
        ...
        switch (unit) {
            ...
            case DATE.WEEK:
                ...
                if (day !== this.weekStartDay) {
                    dt = DATE.add(dt, DATE.DAY, -(day > this.weekStartDay ? (day - this.weekStartDay) : (7 - day - this.weekStartDay)));
                }
                break;
...
Ext.define("Sch.view.Vertical", {
    ...
    onEventUpdate: function (store, model) {
        ...
        this.relayoutRenderedEvents(model.getResource(null, view.eventStore));
    ...
    setColumnWidth: function (width, preventRefresh) {
        ...
        // TODO `columnwidthchange` should be fired from the model, may be relayed by view
...
Ext.define("Sch.plugin.DragSelector", {
    ...
    onStart: function (e) {
        ...
        Sch.util.ScrollManager.register(schedulerView.el);
    },
...
Ext.define("Sch.plugin.ResourceZones", {
    ...
    renderZones: function (resource) {
        ...
        store.each(function (record) {
            ...
            if (record.getResource(null, scheduler.eventStore) === resource && spanStartDate && spanEndDate &&
...
Ext.define("Sch.mixin.TimelineView", {
    ...
    scrollElementIntoView: function (el, container, hscroll, animate, highlight) {
                ...
                animate = Ext.apply({
                    listeners: {
                        afteranimate: function () {
                            el.scrollChildFly.attach(dom).highlight();
                        }
                    }
                }, animate);
            } else {
        ...
        if (newPos != null) {
            el.scrollChildFly.attach(container).scrollTo('top', newPos, animate);
        }
        ...
        if (hscroll !== false) {
            ...
            if (newPos != null) {
                el.scrollChildFly.attach(container).scrollTo('left', newPos, animate);
            }
        }
        return el;
    }
...
Ext.define('Sch.view.TimelineGridView', {
    ...
    onHorizontalScroll: function () {
        ...
        if ((dom.scrollWidth - dom.scrollLeft - width < limit) || dom.scrollLeft < limit) {
            ...
            // If this scroll was triggered by ScrollManager (drag drop of task), we need to cancel the animated scrolling process
            if (SM.proc && SM.proc.el === this.el) {
...
Ext.define('Sch.mixin.AbstractSchedulerView', {
    ...
    _initializeSchedulerView: function () {
        ...
        if (horLayoutCls) {
            this.eventLayout.horizontal = new horLayoutCls({ view: this, timeAxisViewModel: this.timeAxisViewModel });
        }

        if (vertLayoutCls) {
            this.eventLayout.vertical = new vertLayoutCls({ view: this, timeAxisViewModel: this.timeAxisViewModel });
        }
    ...
    scrollEventIntoView: function (eventRec, highlight, animate, callback, scope) {
        ...
        if (Ext.data.TreeStore && this.resourceStore instanceof Ext.data.TreeStore) {
            ...
            if (resources.length > 0 && !resources[ 0 ].isVisible()) {
                resources[ 0 ].bubble(function (node) {
                    node.expand();
                });
            }
        }
...
Ext.define('Sch.mixin.Zoomable', {
    ...
    zoomToLevel: function (level, span, options) {
        ...
        presetCopy.resolutionUnit = Sch.util.Date.getUnitByName(nextZoomLevel.resolutionUnit || bottomHeader.unit);
        presetCopy.resolutionIncrement = nextZoomLevel.resolution;
        ...
        scrollSource.on('scroll', function () {
...
if (!Ext.ClassManager.get("Sch.mixin.TimelinePanel")) {
    ...
    Ext.define('Sch.mixin.TimelinePanel', {
         ...
         *
         * This config defines the width of left and right invisible parts of timespan when {@link #infiniteScroll} set to `true`.
         *
         * It should be provided as a coefficient, which will be multiplied on the width of the scheduling area.
         *
         * For example if `bufferCoef` is `5` and panel view width is 200px then the timespan will be calculated to
         * have approximately 1000px (`5 * 200`) to the left and 1000px to the right of visible area, resulting
         * in 2200px of totally rendered content.
         *
         * The timespan gets recalculated when scroll position reaches limits defined by {@link #bufferThreshold} option.
         *
         */
        bufferCoef: 5,
         ...
         * The limit is calculated as `panelWidth * {@link #bufferCoef} * bufferThreshold`. When, during scrolling, the left or right side
         * has less than that of rendered content - shift is triggered.
         *
         * For example if `bufferCoef` is `5` and panel view width is 200px and `bufferThreshold` is 0.2, then the timespan
         * will be shifted when left or right side has less than 200px (5 * 200 * 0.2) of content.
         */
        bufferThreshold: 0.2,
         ...
         *
         * True to automatically adjust timespan during horizontal scrolling, when scroller comes close to left/right edge.
         *
         * The actually rendered timespan in this mode (and thus the amount of HTML in the DOM) is calculated based
         * on {@link #bufferCoef} option. The moment when the timespan shift happens is determined by the {@link #bufferThreshold} value.
         */
        infiniteScroll: false,
